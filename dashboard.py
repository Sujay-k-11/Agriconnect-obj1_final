...
# ─────────────────────────────────────────────────────────────
if generate:
    price_base   = get_price(state, district, crop)
    ...
    if price_base:
        price_used = price_base*0.8 if "Low" in scenario else (price_base*1.2 if "High" in scenario else price_base)
    else:
        price_used = 2500.0

    yqa  = (yield_kg / 100) / 2.47
    net_cycle = yqa * price_used - base_cost_pa
    net_year  = net_cycle * cycles
    ...
    st.session_state.report = dict(
        ...
        location=location,
        price_used=price_used, yield_kg=yield_kg, cycles=cycles,
...
better       = "🌾 Crop Farming" if total_diff > 0 else "🏠 Leasing"
...
m4.metric("🏆 Best Option",        "Crop Farming" if total_diff > 0 else "Leasing",
          f"Over {years} yr(s)")
